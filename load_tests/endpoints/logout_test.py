    ...
    def on_start(self):
        ...
        with self.client.post("/api/users/signup", json=signup_data, catch_response=True) as response:
            if response.status_code != 201:
                response.failure(f"Signup failed before logout_user: {response.status_code} - {response.text}")
                return
            response.success()
        ...
        with self.client.post("/api/users/login", json=login_payload(signup_data["email"], password), catch_response=True) as login_res:
            if login_res.status_code == 200:
                ...
                login_res.failure(f"Login failed before logout_user: {login_res.status_code} - {login_res.text}")
